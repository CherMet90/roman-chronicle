...
## 🖼️ Иллюстрации: общий шаблон промпта

`[ТЕХНИКА]`: Antique copperplate engraving style, fine line-work, dense cross-hatching, sepia ink on aged paper texture
`[ЭПОХА]`: ссылка на исторический период места действия — указывается в сценарии
`[СЦЕНА]`: краткое описание действия — указывается в сценарии
`[ДЕТАЛИ]`: местные особенности быта, архитектуры, костюмов, атмосферы — указываются в сценарии
`[ЗАПРЕТЫ]`: запрещённые элементы — указываются в сценарии, с акцентом на устранение элементов тех эпох, которые неуместны для сцены

### Шаблон промпта
```
[ТЕХНИКА].[ЭПОХА].[СЦЕНА].[ДЕТАЛИ].[ЗАПРЕТЫ]
```
